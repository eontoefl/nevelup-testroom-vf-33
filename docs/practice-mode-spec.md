...
### 2-5-1. Practice용 DB 함수 시그니처

`supabase-client.js`에 추가할 별도 함수 4개 (기존 V3 함수와 분리):

```js
// 단일 레코드 조회 (대시보드 진입 시 버튼 상태 결정)
async function getStudyResultPractice(userId, sectionType, moduleNumber, practiceNumber)
// → study_results_practice 테이블에서 조회
// → on_conflict 키: (user_id, section_type, module_number, practice_number)

// 실전풀이(initial_record) 저장 — 최초 1회
async function upsertInitialRecordPractice(userId, sectionType, moduleNumber, practiceNumber, recordJson, extras)
// → week/day 대신 practice_number 사용
// → 테이블: study_results_practice
// → on_conflict: 'user_id,section_type,module_number,practice_number'

// 다시풀기(current_record) 저장 — 매번 덮어쓰기
async function upsertCurrentRecordPractice(userId, sectionType, moduleNumber, practiceNumber, recordJson)
// → 테이블: study_results_practice
// → on_conflict: 'user_id,section_type,module_number,practice_number'

// 완료 과제 목록 조회 (ProgressTracker용)
async function getCompletedTasksPractice(userId)
// → study_results_practice에서 initial_record IS NOT NULL 조회
// → 반환: id, section_type, module_number, practice_number
```

4개 모듈 컨트롤러에서의 호출 분기 예시:
```js
// 기존 (정규코스)
var week = ct.currentWeek;
var day = ct.currentDay;
await upsertInitialRecord(user.id, 'reading', mod.moduleNum, week, day, recordJson, extras);

// 연습코스 분기
var cp = window.currentPractice;
if (cp && cp.practiceNumber) {
    await upsertInitialRecordPractice(user.id, 'reading', mod.moduleNum, cp.practiceNumber, recordJson, extras);
} else {
    await upsertInitialRecord(user.id, 'reading', mod.moduleNum, week, day, recordJson, extras);
}
```

### 2-5-2. ProgressTracker 연습코스 캐시 키 구조

정규코스에서 vocab 완료 판단 키: `vocab_w1_월` (week+day 기반)
연습코스에서는 week/day가 없으므로 **practice_number 기반**으로 변경:

| 과제 유형 | 정규코스 캐시 키 | 연습코스 캐시 키 |
|---|---|---|
| R/L/W/S | `reading_13` (section_module) | `reading_13` (동일 — 모듈 번호 13+로 충돌 없음) |
| vocab | `vocab_w1_월` (week_day) | `vocab_p7` (practice_number) |

`ProgressTracker`에서 연습코스 분기:
- `loadCompletedTasks()` → `courseMode`가 `'practice'`이면 `getCompletedTasksPractice()` 호출
- `isTaskNameCompleted()` → vocab일 때 `currentPractice.practiceNumber` 기반 키 사용
- `markCompleted()` → vocab일 때 `vocab_p{N}` 키로 캐시 저장
- `getDayProgress()` → 연습코스에서는 practice_number 기반 진도 계산
...
| 항목 | 내용 |
|---|---|
| **알림 벨** | 연습코스에서도 표시 (정규/연습 무관하게 동일) — 코스 모드와 무관한 개인 알림이므로 |
| **admin-skip** | 연습코스에서도 관리자(test@me.com) 오디오/타이머 스킵 기능 그대로 유지 |

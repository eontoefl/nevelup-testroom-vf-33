...
async function getStudentProgram(userEmail) {
    ...
    const apps = await supabaseSelect(
        'applications',
        `email=eq.${encodeURIComponent(userEmail)}&order=created_at.desc&limit=1&select=id,preferred_program,assigned_program,preferred_start_date,schedule_start,australia_schedule_start,current_step,status,deposit_confirmed_by_admin,practice_enabled,correction_enabled,correction_start_date`
    );
    ...
    let selfPaced = false, selfPacedWeeks = null, selfPacedEndDate = null, selfPacedSchedule = null;
    try {
    ...
    // v2(압축+매일마감): 종료일·확정일정표 컬럼은 더 나중에 추가될 수 있으므로 별도 조회.
    ...
    if (selfPaced) {
        ...
        // 확정 일정표(materialized). 컬럼 부재/미저장 시 null → getSelfPacedSchedule이 실시간 계산으로 대체.
        try {
            const schRows = await supabaseSelect('applications', `id=eq.${app.id}&select=self_paced_schedule`);
            if (schRows && schRows[0]) selfPacedSchedule = schRows[0].self_paced_schedule || null;
...
function _getV3Table(sectionType) {
    if (window.courseMode === 'australia' && (sectionType === 'intro-book-aus' || sectionType === 'vocab' || sectionType === 'reading' || sectionType === 'listening' || sectionType === 'brainstorming' || sectionType === 'intwrt' || sectionType === 'aus-discussion' || sectionType === 'ind-spk' || sectionType === 'intspk')) {
...
/**
 * Practice rewrite_record 저장 (연습코스 해설 화면에서 다시 쓰기)
 * 매번 덮어쓰기 — 최신 결과로 대체
 */
async function upsertRewriteRecordPractice(userId, sectionType, moduleNumber, practiceNumber, rewriteJson) {
    console.log('💾 [Practice] rewrite_record 저장:', sectionType, 'M' + moduleNumber, 'P' + practiceNumber);
    ...
    if (result) {
        console.log('✅ [Practice] rewrite_record 저장 완료:', result.id);
    }
    return result;
}

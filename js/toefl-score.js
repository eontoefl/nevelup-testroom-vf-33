...
function renderToeflScoreList() {
    ...
    sorted.forEach(function(s, idx) {
        ...
        // 회차: 최신순 정렬이므로 (전체 개수 - 인덱스) = 가장 높은 회차가 위에
        var orderLabel = (sorted.length - idx) + '회차';
        var legacyStr = s.legacy_total
            ? '<span class="toefl-legacy">(' + s.legacy_total + '/120)</span>'
            : '';

        html +=
            '<div class="toefl-score-card">' +
                '<div class="toefl-score-card-top">' +
                    '<div class="toefl-score-date">' +
                        '<span class="toefl-order-badge">' + orderLabel + '</span>' +
                        '<span>' + dateStr + '</span>' +
                    '</div>' +
                    '<div class="toefl-score-card-actions">' +
                        (s.score_image
                            ? '<button class="toefl-btn-icon" onclick="openToeflImageViewer(\'' + s.score_image + '\')" title="성적표 보기"><i class="fa-solid fa-image"></i></button>'
                            : '') +
                    ...
                    '</div>' +
                '</div>' +
                '<div class="toefl-score-card-body">' +
                    '<div class="toefl-score-pill"><span class="toefl-pill-label">R</span><span class="toefl-pill-value">' + Number(s.reading).toFixed(1) + '</span></div>' +
                    '<div class="toefl-score-pill"><span class="toefl-pill-label">L</span><span class="toefl-pill-value">' + Number(s.listening).toFixed(1) + '</span></div>' +
                    '<div class="toefl-score-pill"><span class="toefl-pill-label">S</span><span class="toefl-pill-value">' + Number(s.speaking).toFixed(1) + '</span></div>' +
                    '<div class="toefl-score-pill"><span class="toefl-pill-label">W</span><span class="toefl-pill-value">' + Number(s.writing).toFixed(1) + '</span></div>' +
                    '<div class="toefl-score-pill toefl-pill-overall"><span class="toefl-pill-label">Overall</span><span class="toefl-pill-value">' + Number(s.overall).toFixed(1) + '</span>' + legacyStr + '</div>' +
                '</div>' +
                (s.memo
                    ? '<div class="toefl-score-memo"><i class="fa-solid fa-message"></i> ' + escapeHtmlToefl(s.memo) + '</div>'
                    : '') +
            '</div>';
    });
